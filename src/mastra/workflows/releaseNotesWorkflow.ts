...
// Extracts fromRef, toRef, and instructions from the raw commitLogs string,
// then asks an LLM to classify each commit into a structured object.

const parseCommitsStep = createStep({
  id: "parse-commits",
  description: "Parse the commit range commitLogs and classify commits by type using GPT-4o mini",
  ...
  execute: async ({ inputData }) => {
    const lines = inputData.commitLogs.split("\n");
...
const draftStep = createStep({
  ...
  execute: async ({ inputData, getInitData }) => {
    const init     = getInitData<{ commitLogs: string }>();
...
Original request context: ${init?.commitLogs ?? ""}
...
export const releaseNotesWorkflow = createWorkflow({
  ...
  inputSchema: z.object({
    commitLogs: z.string().describe("Commit range (e.g. 'f59ffed..9f130dd') plus optional instructions"),
  }),
  outputSchema: z.object({
    result:  z.string(),
    version: z.string(),
